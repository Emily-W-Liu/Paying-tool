...
export async function saveProductImage(options: {
  bytes: Buffer;
  fileName: string;
  mimeType: string;
  productId: string;
  ...
  const extension = getFileExtension(options.fileName);
  const safeProductId = options.productId.replace(/[^a-zA-Z0-9_-]/g, "-");
  const objectName = `${safeProductId}-${crypto.randomUUID()}${extension}`;
  const config = getSupabaseConfig();
  ...
  return {
    fileName: objectName,
    publicUrl: `${publicBaseUrl}/uploads/products/${objectName}`,
    storage: "local" as const,
  };
}
